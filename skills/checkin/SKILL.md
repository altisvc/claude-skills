...
1. **Skipped Slack inbox** during morning check-in (multiple sessions)
2. **Wrong date on board meeting** — said "today" when it already happened yesterday
3. **Missing tasks from backlog** — omitted items Christopher had to manually point out
...
1. **Skip Phase 1 entirely** — Slack (Altis), Gmail, Calendar, Thoughtful, tasks, and daily logs were already swept overnight
2. **Load staged data** — read the JSON for all pre-gathered context
3. **Run Primary.vc Slack check only** — this is the one source that couldn't run headlessly. Open Chrome, check Primary.vc workspace (Part B of Stage 2), and merge findings into the staged data
4. **Check for meeting-sync staged data** — if `meeting-sync-staged.json` exists with today's date, present meeting decisions BEFORE the check-in questions. Meeting approvals come first because they inform the check-in (e.g., a meeting's follow-ups may be relevant context)
5. **Proceed to Phase 2 (Questions)** — use the staged candidate_questions as the starting set, add any new questions from the Primary.vc check, apply the verification gate to the combined set
6. **After Christopher answers, run Phase 2.5 (Trello Sync Gate)** — staged mode does NOT bypass Trello sync; construct the payload from the overnight sweep plus Christopher's answers, and execute `.claude/scripts/trello-sync.sh` before any briefing
7. After check-in completes, **delete both staged files** to prevent re-processing
...
1. **After Phase 3 briefing, before Phase 4 completeness check**, surface a maintenance nudge:
...
2. Do NOT auto-run the audit or wiki-lint — just surface the summary and let Christopher decide
3. If Christopher says to run them, invoke the skills in sequence (audit first, then wiki-lint)
4. The maintenance staged file is consumed by `/audit` and `/wiki-lint` when they detect it — those skills handle their own cleanup
...
1. Determine today's actual date and day of week
2. Store internally — do NOT output to Christopher yet
...
5. Open Primary.vc Slack in Chrome browser: navigate to `https://app.slack.com/client/T03KQRA9L` → if prompted, click "use Slack in your browser"
6. Check recent DMs — especially threads with Ben, Emily, Tanmaye, Nick, and any active deal-related conversations
7. Scan key Primary channels: #investmentteam, #altis-primary-gtm, and any channels with unread indicators
8. Search for Christopher's name or "Altis" in recent messages to catch action items directed at him
...
9. Fetch recent threads via `GET /api/v1/threads` using `THOUGHTFUL_API_KEY` env var (set in `~/.zshrc`)
   ```bash
   curl -s -H "Authorization: Bearer $THOUGHTFUL_API_KEY" https://www.thoughtful.app/api/v1/threads | jq '.[] | {title, updated_at}' | head -40
   ```
10. Note any active/recent threads from the last 24 hours — these represent work Christopher is doing outside of Slack/vault
11. Flag any threads that might generate action items or relate to open tasks
12. **Fallback only:** If API returns an error or auth fails, fall back to Chrome browser (`https://www.thoughtful.app/dashboard`)
...
13. Run inbox triage for unread/starred messages:
    ```bash
    gws gmail +triage
    ```
14. Run targeted searches for 3-5 known open threads from recent daily logs:
    ```bash
    gws gmail users messages list --params '{"userId":"me","q":"from:specific@email.com newer_than:3d"}'
    ```
15. Check for replies to recent outbound (prospects, counsel, expert network contacts)
16. Gmail is **READ-ONLY** — no sending, modifying, labeling, or deleting. Christopher uses Spark Mail for email actions.
17. **If `gws` auth expires:** `gws auth login -s calendar,gmail`
...
1. Fetch today's events:
   ```bash
   gws calendar events list --params '{"calendarId":"primary","timeMin":"YYYY-MM-DDT00:00:00-04:00","timeMax":"YYYY-MM-DDT23:59:59-04:00","singleEvents":true,"orderBy":"startTime"}'
   ```
2. Fetch next working day's events (if today is Friday, query through Monday):
   ```bash
   gws calendar events list --params '{"calendarId":"primary","timeMin":"NEXT_DAY_START","timeMax":"NEXT_DAY_END","singleEvents":true,"orderBy":"startTime"}'
   ```
3. For each event, classify:
...
4. For meetings with others, assess: prep needed? Prior Slack context? Open action items with this person?
5. For solo time blocks, note the intended task — these represent commitments Christopher made to himself
6. **Fallback:** If `gws` auth fails, open Chrome and navigate to `https://calendar.google.com`
7. **If `gws` auth expires:** `gws auth login -s calendar,gmail`
...
1. Read `$MAIN_REPO/02-projects/operations/christopher-tactical-todos.md`
2. Read `$MAIN_REPO/02-projects/operations/ms-claudia-backlog.md`
3. Scan Slack messages from Stage 2 for commitments not yet captured (look for: "I'll send," "action item," "let me follow up," "TODO," "need to," assignments to Christopher)
4. Check Granola recent meetings for action items (use `mcp__granola__find_recent_meetings` — last 3 days)
5. Cross-reference solo time blocks from Stage 3 against task list — if a block has no corresponding task, flag
6. Flag any items not updated in 5+ days as STALE
7. Flag any items due today or overdue as URGENT

**Part B: Daily Log Implications Scan**

8. Read the **last 5 daily logs** from `$MAIN_REPO/06-daily-log/` (by date, most recent first)
9. For each log, focus on:
   - **`## Forward Implications`** sections — these explicitly name downstream effects of that day's work
   - **`**Open threads:**`** items — anything unresolved that may collide with today's calendar or upcoming deliverables
   - **`**Outcomes:**`** items — recent changes to files, positioning, architecture, or processes that could affect upcoming commitments
10. **Cross-reference recent changes against upcoming commitments:** For every calendar event, deliverable, or task due in the next 48 hours, ask: *Did anything change in the last 5 days that affects this?* Specifically:
...
11. Surface any collisions or stale assumptions as **questions in Phase 2**, not just FYIs in the briefing.
...
- Use `mcp__Claude_in_Chrome__computer` (browser tool) for rendering
- Do NOT default to WebFetch for sites requiring JavaScript
...
- Primary: `gws calendar events list` with date-bounded params
- Fallback: `https://calendar.google.com` via Chrome if `gws` auth fails
- Auth fix: `gws auth login -s calendar,gmail`
- Check today + next working day (Friday → check Monday)

Task files read during Stage 4 (all paths relative to `$MAIN_REPO`):
- `$MAIN_REPO/02-projects/operations/christopher-tactical-todos.md`
- `$MAIN_REPO/02-projects/operations/ms-claudia-backlog.md`
- `$MAIN_REPO/02-projects/operations/altis-backlog-tracker.md` (for strategic context)
- `$MAIN_REPO/02-projects/operations/strategic-big-rocks.md` (for prioritization framing)
...
- API: `GET /api/v1/threads` with `THOUGHTFUL_API_KEY` (set in `~/.zshrc`)
- Note: API returns thread metadata only (no message content) — sufficient for WIP detection
- Fallback: `https://www.thoughtful.app/dashboard` via Chrome if API auth fails
- Check: recent threads (last 24 hours) for active drafting, WIP, task-related threads
